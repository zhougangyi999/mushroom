...
var cardType,backCode;
function backcardCheck(cardNo) {
	for(var i = 0; i < bankcardList.length; i++) {
		for(var j = 0; j < bankcardList[i].patterns.length; j++) {
			/*alert("名"+bankcardList[i].bankCode+"正"+bankcardList[i].patterns[j].reg)*/
			if(bankcardList[i].patterns[j].reg.test(cardNo)) {
				cardType=bankcardList[i].patterns[j].cardType;
				backCode=bankcardList[i].bankCode;
				return true;
			}
		}
